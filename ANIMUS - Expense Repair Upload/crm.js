...
document.addEventListener("click", (event) => {
  ...
  if (!button) return;
  if (button.dataset.freshExpenseOpen) {
    event.preventDefault();
    event.stopImmediatePropagation();
    freshExpenseOpenReceipt(button.dataset.freshExpenseOpen);
    return;
  }
  if (button.dataset.freshExpenseDelete) {
    event.preventDefault();
    event.stopImmediatePropagation();
    freshExpenseDeleteReceipt(button.dataset.freshExpenseDelete);
    return;
  }
