...
(() => {
  ...
  function kpi(icon, tone, title, value, note, field, valueTone = "") { return `<article class="animus-revenue-kpi"><span class="animus-revenue-kpi-icon ${tone}">${icon}</span><div><small>${title}</small><strong class="${valueTone}">${value}</strong><em>${note}</em></div>${field ? `<button class="revenue-edit-kpi" data-revenue-kpi-edit="${field}" title="Review ${title}">✎</button>` : ""}</article>`; }
  ...
  function render() {
    ...
    view.innerHTML = `<section class="animus-revenue-center">
      ...
      <section class="animus-revenue-card"><header class="animus-revenue-card-head"><div><h2>Job Profitability</h2><p>Every value is tied to an existing ANIMUS work file.</p></div><div class="animus-revenue-table-tools"><label class="animus-revenue-search">⌕<input id="animusRevenueSearch" value="${safe(state.query)}" placeholder="Search jobs or customers..."></label><div class="animus-revenue-columns"><button id="animusRevenueColumns">Columns ▾</button>${state.columnsOpen ? `<div class="animus-revenue-columns-menu">${Object.entries({date:"Date",margin:"Profit Margin",expenses:"Expenses",actions:"Actions"}).map(([key,label]) => `<label><input type="checkbox" data-revenue-column="${key}" ${state.columns[key] ? "checked" : ""}> ${label}</label>`).join("")}</div>` : ""}</div></div></header>
        <div class="animus-revenue-table-wrap"><table class="animus-revenue-table"><thead><tr><th class="revenue-col-date${state.columns.date ? "" : " hidden"}">Date</th><th>File Name (Job)</th><th>Gross Earnings</th><th class="revenue-col-expenses${state.columns.expenses ? "" : " hidden"}">Expenses</th><th>Labor</th><th>Net Profit</th><th class="revenue-col-margin${state.columns.margin ? "" : " hidden"}">Profit Margin</th><th class="revenue-col-actions${state.columns.actions ? "" : " hidden"}">Actions</th></tr></thead><tbody>${pageRows.length ? pageRows.map(tableRow).join("") : `<tr><td colspan="8" class="animus-revenue-empty">No matching job profitability rows.</td></tr>`}<tr class="animus-revenue-total"><td class="revenue-col-date${state.columns.date ? "" : " hidden"}"></td><td>Totals</td><td>${money(total.gross)}</td><td class="revenue-col-expenses${state.columns.expenses ? "" : " hidden"}">${money(total.expenses)}</td><td>${money(total.labor)}</td><td class="animus-revenue-money net ${toneForValue(total.profit)}">${money(total.profit)}</td><td class="revenue-col-margin${state.columns.margin ? "" : " hidden"} ${toneForValue(margin)}">${margin.toFixed(1)}%</td><td class="revenue-col-actions${state.columns.actions ? "" : " hidden"}"></td></tr></tbody></table></div>
        ${auditMarkup()}
