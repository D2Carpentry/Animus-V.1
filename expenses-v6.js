...
(() => {
  ...
  function visibleEntries() {
    ...
    return expenseEntries().filter((entry) => {
      ...
      const itemDetails = Array.isArray(entry.items) ? entry.items.flatMap((item) => [item?.name, item?.price, item?.category]) : [];
      return [entry.vendor, entry.title, entry.imageTitle, entry.category, entry.paymentType, entry.date, entry.amount, entry.receiptImageKey, entry.file?.clientName, entry.file?.fileNumber, entry.notes, ...itemDetails].some((value) => String(value || "").toLowerCase().includes(query));
    });
  }
  ...
  function drawerMarkup() {
    ...
    const receiptPreview = imageSrc
      ? (isPdfReceipt(data)
        ? `<div class="expense-preview expense-pdf-preview"><object data="${esc(imageSrc)}" type="application/pdf" aria-label="Receipt PDF preview"><a href="${esc(imageSrc)}" target="_blank" rel="noopener">Open saved PDF receipt</a></object><a class="expense-pdf-open" href="${esc(imageSrc)}" target="_blank" rel="noopener">Open full PDF</a></div>`
  ...
  function render() {
    ...
    bind();
    if (state.searchCursor !== null) {
      const search = document.querySelector("#expenseSearch");
      if (search) {
        search.focus();
        const cursor = Math.min(state.searchCursor, search.value.length);
  ...
  function captureDrawer() {
    ...
    document.querySelectorAll("[data-expense-item-quantity]").forEach((input) => { draft.items[Number(input.dataset.expenseItemQuantity)].quantity = input.value; });
  ...
  async function scan(file) {
    ...
    state.draft = cleanDraft({ fileId: targetFileId, title:file.name, imageTitle:file.name, receiptContentType:file.type || "" });
  ...
  function showPriceImport(items) {
    ...
    modal?.querySelector(".expense-section-actions [data-expense-import-submit]")?.remove();
    modal?.querySelectorAll("[data-expense-import-submit]").forEach((button) => {
