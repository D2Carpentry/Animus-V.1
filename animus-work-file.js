...
(function () {
  ...
  function detailsPanel(file) {
    ...
    return `<div class="animus-tab-panel"><section class="animus-info-card"><div class="animus-card-head"><h3>Job Details</h3><button class="animus-record-button primary small" data-animus-edit="details">Edit Job Details</button></div><div class="animus-detail-grid">${groups.map(([title, rows]) => `<section class="animus-detail-card"><h4>${escape(title)}</h4><dl class="animus-info-list">${rows.map(([label, value]) => `<div class="animus-info-row"><div><dt>${escape(label)}</dt><dd>${escape(value)}</dd></div></div>`).join("")}</dl></section>`).join("")}</div></section></div>`;
  }
