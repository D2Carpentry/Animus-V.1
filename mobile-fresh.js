...
const LOCAL_KEY = "animusMobileFreshStateV1";
...
function saveLocal() { localStorage.setItem(LOCAL_KEY, JSON.stringify({ state, activeFileId })); }
function readLocal() { try { const saved = JSON.parse(localStorage.getItem(LOCAL_KEY) || "{}"); if (saved.state) { state = saved.state; activeFileId = saved.activeFileId || ""; } } catch (_) {} }
...
readLocal(); renderAll(); loadCloud().catch(() => { notify("Offline copy",true); renderAll(); });
