...
function movePhoto(id, direction) {
  ...
  if (index < 0 || nextIndex < 0 || nextIndex >= state.photos.length) return;
  [state.photos[index], state.photos[nextIndex]] = [state.photos[nextIndex], state.photos[index]];
  renderPhotos();
  updatePreview();
}
...
function renderPhotos() {
  ...
  state.photos.forEach((photo) => {
    ...
    card.innerHTML = `
      ...
      <div class="photo-move-controls" aria-label="Move invoice image">
        <button type="button" data-action="move-up" title="Move image up" aria-label="Move image up">&#8593;</button>
        <button type="button" data-action="move-down" title="Move image down" aria-label="Move image down">&#8595;</button>
      </div>
    ...
    card.addEventListener("click", (event) => {
      ...
      if (event.target.dataset.action === "move-up") movePhoto(photo.id, -1);
      if (event.target.dataset.action === "move-down") movePhoto(photo.id, 1);
