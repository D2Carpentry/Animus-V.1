...
function openMobileNoteSheet() {
  ...
  $("saveMobileNote")?.addEventListener("click", async () => {
    ...
    closeSheet();
    renderDetail();
    try { await saveFilePatch(file, { notes:file.notes, timeline:file.timeline }); notify("Note saved to cloud"); } catch (error) { notify("Note save failed", true); window.alert(error.message || "The note could not be saved."); }
  });
}
...
async function saveReviewedWorkPhotos() {
  ...
  try {
    ...
    await saveFilePatch(file, { workPhotos:file.workPhotos, timeline:file.timeline });
    workPhotoDrafts = [];
    closeSheet();
...
function updateRevenueExpense(file) { const total = fileExpenseTotal(file); let row = state.revenue.find((entry) => entry.dashboardFileId === file.id || entry.fileNumber === file.fileNumber); if (!row) return null; row.expenses = total; row.profit = parseMoney(row.gross) - parseMoney(row.expenses) - parseMoney(row.labor); return row; }
...
$("saveButton").addEventListener("click", async () => { const button=$("saveButton"); button.disabled=true; button.textContent="Refreshing"; try { await loadCloud(); notify("Live cloud data refreshed"); } catch (error) { notify("Refresh failed",true); window.alert(error.message); } finally { button.disabled=false; button.textContent="Refresh"; } });
